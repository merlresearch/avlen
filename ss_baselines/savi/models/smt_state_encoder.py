...
class SMTStateEncoder(nn.Module):
    ...
    def __init__(
        ...
        '''
        if use_query_count:
            print('using query count')
            print(input_size, query_count_emb_size)
            input_size += query_count_emb_size
    ...
    def single_forward(self, x, memory, memory_masks, goal=None):
        ...
        # check: It shouldn't be true for dialog training
        ...
        if self._use_pose_encoding:  #True
            pi, pj = self._pose_indices
            x_pose = x[..., pi:pj]
            memory_poses = memory[..., pi:pj]
            ...
            x = torch.cat([x[..., :pi], x_pose_enc, x[..., pj:]], dim=-1)
            memory = torch.cat([memory[..., :pi], memory_poses_enc, memory[..., pj:]], dim=-1)
            del memory_poses, x_pose, x_pose_enc, memory_poses_enc
        ...
        '''
        if from_eval:
            print(f"Used GPU memory before transformer- {nvidia_smi.getInstance().DeviceQuery('memory.used')}")
        '''    
        if goal is not None: 
        ...
        '''
        if from_eval:
            print(f"Used GPU memory after transformer- {nvidia_smi.getInstance().DeviceQuery('memory.used')}")
        '''
        return x_att
