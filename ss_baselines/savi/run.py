...
def main():
    ...
    if not os.path.exists(args.model_dir+'/data/vln'):
        os.makedirs(args.model_dir+'/data/vln', exist_ok=True)
    ...
    if config.DIALOG_TRAINING:
        config.defrost()
        config.TASK_CONFIG.ENVIRONMENT.MAX_EPISODE_STEPS = config.NUM_DIALOG_STEPS
        config.freeze()
