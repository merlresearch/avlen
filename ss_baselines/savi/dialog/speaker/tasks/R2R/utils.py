...
def load_datasets(splits, subinstructions=False):
    data = []
    for split in splits:
        if not subinstructions:
            with open('./ss_baselines/savi/dialog/speaker/tasks/R2R/data/R2R_%s.json' % split) as f:
                data += json.load(f)
        else:
            with open('./ss_baselines/savi/dialog/speaker/tasks/R2R/data/FGR2R_%s.json' % split) as f:
                data += json.load(f)
...
def try_cuda(pytorch_obj):
    import torch.cuda
    from speaker_pipeline import args
    try:
    ...
    if torch.cuda.is_available() and not disabled:
        if args.device is None:
            ...
            return pytorch_obj.to(args.device)            
    else:
        return pytorch_obj
