...
def generate_video(
    ...
    if "disk" in video_option:
        ...
        if audios is None:
            ...
            images_to_video_with_audio(images, video_dir, video_name, audios, sr, fps=fps, text=text, num_steps=num_steps )
...
def plot_top_down_map(info, dataset='replica', pred=None):
    ...
    if pred is not None:
        ...
        source_rotation = quat_from_angle_axis(np.deg2rad(info["top_down_map"]["agent_angle"]), np.array([0, 1, 0]))
...
def images_to_video_with_audio(
    ...
    **kwargs
    ...
    if not os.path.exists(os.path.join(output_dir,'cv2')):
        os.makedirs(os.path.join(output_dir,'cv2'))

    video_name = video_name.replace(" ", "_").replace("\n", "_") + ".mp4"

    if not os.path.exists(os.path.join(output_dir,'cv2', video_name)):
        os.makedirs(os.path.join(output_dir,'cv2', video_name))
    ...
    amplitude_scaling_factor = 10 #100
    ...
    # adding text
    if text != None:
        my_text = mpy.TextClip(text, fontsize=20, color='white')
        my_text = my_text.set_duration(num_steps)
        video_with_new_audio = mpy.CompositeVideoClip([video_with_new_audio, my_text])
    '''
    video_clip.write_videofile(os.path.join(output_dir, video_name))

    # For inserting text
    assert len(images)==len(text), 'image and text length are not same'
    # inserting text
    cap = cv2.VideoCapture(os.path.join(output_dir, video_name))
    frame_width = int(cap.get(3))
    frame_height = int(cap.get(4))
    frame_size = (frame_width,frame_height)
    output = cv2.VideoWriter(os.path.join(output_dir, 'cv2', video_name, video_name), cv2.VideoWriter_fourcc('M','J','P','G'), fps, frame_size)

    #print(len(images),'len(images)', video_name)
    # sys.exit()
    ...
    for cnt in range(len(images)):
        ...
        if ret:
            if cnt == len(images)-2:
                cv2.imwrite(os.path.join('/home/sudipta/isavi/dialog_audionav/data/models/from_anoop/savi_15_5/run_2/video_dir', 'cv2', video_name, '{}.jpg'.format(cnt)), frame)

            if text[cnt]:
                cv2.imwrite(os.path.join('/home/sudipta/isavi/dialog_audionav/data/models/from_anoop/savi_15_5/run_2/video_dir', 'cv2', video_name, '{}.jpg'.format(cnt)), frame)
                (w, h), _ = cv2.getTextSize(text[cnt], font, fontscale, font_thickness)
                cv2.rectangle(frame, (x1-10, y1-35), (x1+w+10, y1+h+5), (0, 255, 255), -1)
                cv2.putText(frame,
                        text[cnt],
                        (x1, y1),
                        font, fontscale,
    ...
    video_clip = mpy.VideoFileClip(os.path.join(output_dir, 'cv2', video_name, video_name))
