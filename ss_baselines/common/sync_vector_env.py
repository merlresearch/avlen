    ...
    def __call__(self, command, data):
        while command != CLOSE_COMMAND:
            ...
            # --------------------------
                ...
                return self._env.agent_state()

            elif command == IS_NEW_EPISODE_COMMAND:
                return self._env._new_episode

            elif command == ORACLE_ACTION_COMMAND:
                return self._env.compute_oracle_actions()
...
class SyncVectorEnv:
    ...
    def agent_state(self):
        results = [worker(STATE_COMMAND, None) for worker in self.workers]
        return results

    def is_new_episode(self):
        results = [worker(IS_NEW_EPISODE_COMMAND, None) for worker in self.workers]
        return results

    def compute_oracle_actions(self):
        results = [worker(ORACLE_ACTION_COMMAND, None) for worker in self.workers]
        return results
