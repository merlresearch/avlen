...
class SoundSpacesSim(Simulator, ABC):
    ...
    def __init__(self, config: Config) -> None:
        ...
        # -----------------------------------------------
        # print(self._sensor_suite.sensors.values())
        # -----------------------------------------------
        ...
        # -----------------------------------------------
        # print('self.sim_config.agents[0].action_space', self.sim_config.agents[0].action_space)
        # -----------------------------------------------
        ...
        # -------------------------------------
        # also need to compute shortest path for all the points
        ...
        with open('./data/node2view.json', 'r') as f:
            self._node2view = json.load(f)


        # ----------------------------------------------------------------------
        # Should initialize the dialog models here
        # ---------
        # ques gen
        # self.ques_module = Ques_Gen()
    ...
    def reconfigure(self, config: Config) -> None:
        ...
        # dialog pretraining
        self._sub_instr = self.config.AGENT_0.SUB_INSTR
    ...
    def step(self, action, only_allowed=True):
        ...
        self._previous_step_collided = False
        # STOP: 0, FORWARD: 1, LEFT: 2, RIGHT: 3, # QUERY: 9 (need to change it to 4) (or no need?)
        # need to update the action space (specify distance and angle since we are using different action space)
        if action == HabitatSimActions.STOP:
            ...
            '''
            # -----------------------------------------
            elif action == HabitatSimActions.QUERY:
                # print('action for query', action)
                # -------------------------------------
                # print('QUERY action')
                # check to generate ques
                # get observation
                # sim_obs = self._get_sim_observation()
                # print(sim_obs['rgb'])
                # print(self.ques_module.ques_out(sim_obs['rgb']))
                # sys.exit()

                # for sanity check we are cosidering two forward steps
                forward_cnt = 0
    ...
    def _load_single_distractor_sound(self):
        # print('self._source_sound_dict', self._source_sound_dict)
        # print('self._current_distractor_sound', self._current_distractor_sound)
