...
# Adapted from semantic_audionav_dataset.py

# The MIT license below is in the original source at https://github.com/facebookresearch/sound-spaces/blob/main/soundspaces/datasets/semantic_audionav_dataset.py
...
from soundspaces.tasks.semantic_audiodialognav_task import SemanticAudioGoalDialogNavEpisode, SemanticAudioGoal, \
    ObjectViewLocation
...
@registry.register_dataset(name="SemanticAudioDialogNav")
class SemanticAudioDialogNavDataset(Dataset):
    category_to_task_category_id: Dict[str, int]
    category_to_scene_annotation_category_id: Dict[str, int]
    episodes: List[SemanticAudioGoalDialogNavEpisode]
    ...
    def get_scenes_to_load(config: Config, **kwargs) -> List[str]:
        ...
        episodes.
        """
        assert SemanticAudioDialogNavDataset.check_config_paths_exist(config), \
        ...
        cfg.defrost()
        cfg.CONTENT_SCENES = []
        dataset = SemanticAudioDialogNavDataset(cfg)
        return SemanticAudioDialogNavDataset._get_scenes_from_folder(
    ...
    def dedup_goals(dataset: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, ep in enumerate(dataset["episodes"]):
            ...
            ep = SemanticAudioGoalDialogNavEpisode(**ep)
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        if ALL_SCENES_MASK in scenes:
            scenes = SemanticAudioDialogNavDataset._get_scenes_from_folder(
    ...
    def from_json(
        ...
        for i, episode in enumerate(deserialized["episodes"]):
            episode = SemanticAudioGoalDialogNavEpisode(**episode)
