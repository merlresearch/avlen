...
class SemanticAudioNavDataset(Dataset):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        for scene in scenes:
            ...
            '''
            # for debug
            episode_ids[self.episodes[-1].scene_id] = []
            '''
        ...
        if len( self.episodes)>0:
            ...
            # sys.exit()
            for elem in self.episodes:
                if elem.episode_id not in episode_ids:
                    episode_ids[elem.scene_id].append(elem.episode_id)

            for scene, epi in episode_ids.items():
                t_cnt += len(epi)
            print('total episode ids', t_cnt)
        '''
