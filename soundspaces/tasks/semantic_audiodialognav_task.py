...
# Adapted from semantic_audionav_task.py

# The MIT license below is in the original source at https://github.com/facebookresearch/sound-spaces/blob/main/soundspaces/tasks/semantic_audionav_task.py
# although the sound-spaces package is licensed as CC-BY-4.0
...
class SemanticAudioGoalDialogNavEpisode(NavigationEpisode):
    r"""AudioGoal-Dialog Navigation Episode
    """
    object_category: str
    sound_id: str
    ...
    # extra info to add for dialog
    dialog_node: str = attr.ib(default=None, validator=not_none_validator)
    dialog_point: List[float] = attr.ib(default=None, validator=not_none_validator)
    sub_instr: str = attr.ib(default=None, validator=not_none_validator)
    dialog_rotation: List[float] = attr.ib(default=None, validator=not_none_validator)
...
class SemanticAudioGoalDialogSensor(Sensor):
    ...
    def get_observation(
        ...
        observations,
        *args: Any,
        episode: SemanticAudioGoalDialogNavEpisode,
        **kwargs: Any,
    ) -> Optional[int]:
        if self.config.GOAL_SPEC == "TASK_CATEGORY_ID":
...
class SemanticAudioDialogNavigationTask(NavigationTask):
...
def merge_sim_episode_config(
    ...
    if (
        ...
        # for dialog
        agent_cfg.DIALOG_NODE = episode.dialog_node
        agent_cfg.DIALOG_POINT = episode.dialog_point
        agent_cfg.SUB_INSTR = episode.sub_instr
        agent_cfg.DIALOG_ROTATION = episode.dialog_rotation
        agent_cfg.freeze()
    return sim_config
