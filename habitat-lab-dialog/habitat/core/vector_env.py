...
# ---------- 
STATE_COMMAND = 'state'
IS_NEW_EPISODE_COMMAND = 'is_new_episode'
ORACLE_ACTION_COMMAND = 'o_action'
QUERY_NUM_COMMAND = 'query_num'
IS_QUERIED_NUM_COMMAND = 'is_queried'
ENV_ID_COMMAND = 'which_env'
CONSTRAINT_COMMAND = 'cons_constraint'
...
class VectorEnv:
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                ...
                # --------------------------    
                    ...
                    connection_write_fn(env.agent_state())
                    
                elif command == IS_NEW_EPISODE_COMMAND:
                    connection_write_fn(env._new_episode)

                elif command == ORACLE_ACTION_COMMAND:
                    connection_write_fn(env.compute_oracle_actions())

                # ----------------------------

                elif command == QUERY_NUM_COMMAND:
                    connection_write_fn(env.set_query_num(data))

                
                elif command == CONSTRAINT_COMMAND:
                    connection_write_fn(env.set_constraint_reward(data))

                elif command == IS_QUERIED_NUM_COMMAND:
                    connection_write_fn(env.set_idx(data))

                elif command == ENV_ID_COMMAND:
                    connection_write_fn(env.set_is_queried(data))
    ...
    def compute_oracle_actions(self):
        for write_fn in self._connection_write_fns:
            write_fn((ORACLE_ACTION_COMMAND, None))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        return results

    def set_query_num_at(self, index_env: int, query_num: int):
        self._connection_write_fns[index_env]((QUERY_NUM_COMMAND, query_num))
        self._connection_read_fns[index_env]()

    def set_constraint_reward_at(self, index_env: int, constraint_reward: float):
        self._connection_write_fns[index_env]((CONSTRAINT_COMMAND, constraint_reward))
        self._connection_read_fns[index_env]()
        
    def set_idx_at(self, index_env: int, env_id: int):
        self._connection_write_fns[index_env]((IS_QUERIED_NUM_COMMAND, env_id))
        self._connection_read_fns[index_env]()

    def set_is_queried_at(self, index_env: int, is_queried: bool):
        self._connection_write_fns[index_env]((ENV_ID_COMMAND, is_queried))
        self._connection_read_fns[index_env]()
    
    def set_query_num(self, query_num_list: List[Any]):
        for index_env, query_num in enumerate(query_num_list):
            self.set_query_num_at(index_env, query_num)

    def set_constraint_reward(self, constraint_reward_list: List[Any]):
        for index_env, constraint_reward in enumerate(constraint_reward_list):
            self.set_constraint_reward_at(index_env, constraint_reward)

        
    def set_idx(self, index_env_list: List[Any]):
        for index_env, env_id in enumerate(index_env_list):
            self.set_idx_at(index_env, env_id)
        
    def set_is_queried(self, is_queried_list: List[Any]):
        for index_env, is_queried in enumerate(is_queried_list):
    ...
    def is_new_episode(self):
        for write_fn in self._connection_write_fns:
            write_fn((IS_NEW_EPISODE_COMMAND, None))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        return results
    ...
    '''
    def shortest_path_actions(self):
        for write_fn in self._connection_write_fns:
            write_fn((ORACLE_ACTION_COMMAND, None))
        results = []
        for read_fn in self._connection_read_fns:
